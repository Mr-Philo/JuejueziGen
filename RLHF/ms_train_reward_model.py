...
from ms_model import RewardModel, compute_rank_list_loss
...
def process_dataset(source, tokenizer, pad_value, max_seq_len=64, batch_size=32, shuffle=True):
    ...
    rename_columns = ["input_ids", "score"]         # 必须要将输入的列名改为input_ids，否则Trainer会报错
    ...
    type_cast_op = mindspore.dataset.transforms.TypeCast(mindspore.float32)
    
    # map dataset
    dataset = dataset.map(operations=[tokenizer, pad_op], input_columns="text")     # 对文本tokenize+pad
    dataset = dataset.map(operations=[type_cast_op], input_columns="score")         # 对分数转换为float32
    # # rename dataset
